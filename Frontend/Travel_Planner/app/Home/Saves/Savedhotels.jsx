...
const SavedAttractionsScreen = () => {
  ...
  const fetchAttractions = async () => {
    try {
      const response = await axios.post(
        'https://sunbeam-pet-octopus.ngrok-free.app/getHotels',
        {},
        {
  ...
  const deleteAttraction = async (name) => {
    try {
      const response = await axios.post(
        'https://sunbeam-pet-octopus.ngrok-free.app/deleteHotels',
        { name },
      ...
      if (response.status === 200) {
        ...
        Alert.alert('Error', response.data.error || 'Failed to delete hotels.');
      }
    } catch (error) {
      console.error('Error deleting Hotels:', error);
      Alert.alert('Error', 'An error occurred while deleting the Hotels.');
    }
  };
    ...
    <View style={styles.cardHeader}>
      ...
      <View style={styles.iconRow}>
        ...
        <TouchableOpacity
            onPress={async () => {
              try {
                const response = await addPlaceToAnotherList(item.name, 'hotel', 'saved');
                if (response) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Saved Hotels</Text>
      {attractions.length > 0 ? (
        ...
        <Text style={styles.emptyText}>No saved Hotels found.</Text>
      )}
    </View>
  );
};
