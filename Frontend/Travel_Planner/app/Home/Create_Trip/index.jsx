...
const UserPlacesScreen = () => {
  ...
  const fetchPlaces = async () => {
    try {
      const response = await axios.get('http://192.168.57.138:5000/get_user_places', {
  ...
  const checkIfVisited = async (place_id, place_type) => {
    try {
      const response = await axios.get('http://192.168.57.138:5000/check_place_visited', {
  ...
  const markAsVisited = async (place_id, place_type) => {
    try {
      const response = await axios.post(
        'http://192.168.57.138:5000/mark_as_visited',
        { place_id, place_type },
        { withCredentials: true }
      );
  ...
  const deletePlace = async (place_id, place_type) => {
    try {
      const response = await axios.delete('http://192.168.57.138:5000/delete_place', {
  ...
  const deleteAllPlaces = async () => {
    try {
      const response = await axios.delete('http://192.168.57.138:5000/delete_all_places', {
        withCredentials: true,
      });
      Alert.alert('Success', response.data.message);
      fetchPlaces(); // Refresh the list after deletion
  ...
  const getPlaceCoordinates = async (place_id) => {
    try {
      const response = await axios.get(
        `http://192.168.57.138:5000/get_user_places_locations?place_id=${place_id}`,
        { withCredentials: true }
      );
